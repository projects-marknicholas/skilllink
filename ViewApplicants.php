...
<body onload="logoBeat()" style="font-family: 'Sora', sans-serif;">
    ...
    ?>
    <!-- Main Container -->
    <div class="container-fluid" style="background-color: #FE0089; padding-left: 50px; padding-right: 50px;">
        <?php
        include 'connect.php';
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <button class="btn-statuss accept" name="status">Accept</button>
                                            <button class="btn-statuss reject" name="status">Reject</button>
                                            <?php else: ?>
                                            <!-- Display nothing -->
                                            <?php endif; ?>
                                        </td>
    ...
    <script>
    $(document).on('click', '.btn-statuss', function () {
        // Get the status from the button class (accept or reject)
        const newStatus = $(this).hasClass('accept') ? 'Accepted' : 'Rejected';

        // Get the parent row and extract the pid and sid
        const row = $(this).closest('tr');
        const pid = row.find('td:nth-child(1)').text();
        const sname = row.find('td:nth-child(2)').text();

        // Send the data to the backend using AJAX
        $.ajax({
        url: 'updateStatus.php',
        type: 'POST',
        data: { pid, sname, status: newStatus },
